...
public class RubotoOnPlaybackPositionUpdateListener implements android.media.AudioTrack.OnPlaybackPositionUpdateListener {
  ...
  public void onMarkerReached(android.media.AudioTrack track) {
    if (callbackProcs[CB_MARKER_REACHED] != null) {
      try {
        RuntimeHelpers.invoke(getRuby().getCurrentContext(), callbackProcs[CB_MARKER_REACHED], "call" , JavaUtil.convertJavaToRuby(getRuby(), track));
  ...
  public void onPeriodicNotification(android.media.AudioTrack track) {
    if (callbackProcs[CB_PERIODIC_NOTIFICATION] != null) {
      try {
        RuntimeHelpers.invoke(getRuby().getCurrentContext(), callbackProcs[CB_PERIODIC_NOTIFICATION], "call" , JavaUtil.convertJavaToRuby(getRuby(), track));
      } catch (RaiseException re) {
        re.printStackTrace();
      }
    }
